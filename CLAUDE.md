...
- The "Bot" checkbox in local (hotseat) games (`app/src/bot/useBotTurn.ts`) is powered by a pure, deterministic expected-value (EV) engine in `packages/game-engine/src/bot/` (`chooseBotRollDecision`/`chooseBotScoreDecision`) — it exhaustively evaluates all 32 hold masks and every possible reroll outcome each decision, with no network dependency or external CLI. An earlier prototype proxied bot decisions to the `claude` CLI in headless mode via a local `bot-server/` workspace; that path was removed once the EV engine replaced it (see `docs/superpowers/specs/2026-07-08-bot-faza-szkolki-heurystyka-design.md` and the earlier `2026-07-07-bot-silnik-ev-design.md` for the full rationale).
