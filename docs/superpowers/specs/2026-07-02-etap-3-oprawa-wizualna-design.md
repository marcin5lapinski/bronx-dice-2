...
- **Animacja przy rzucie:** kliknięcie "Rzuć kośćmi" animuje wizualnie każdą kość, która NIE jest zatrzymana (dokładnie te, które silnik faktycznie przerzuca — spójne z `rollDice`/`rollInTurn` z Etapu 1/2). Mechanizm: przez ok. 400–600ms kość "miga" losowymi wartościami 1–6 w szybkim tempie (np. co 60–80ms), a na koniec animacji zatrzymuje się na finalnej wartości zwróconej przez silnik. Wybrany celowo zamiast obrotu/skalowania, bo działa identycznie dla cyfr i dla przyszłych grafik kości (podmiana obrazka zamiast cyfry w tej samej pętli). To lokalny stan UI w `DiceTray`/`GameScreen` (np. klasa CSS `.rolling` + tymczasowa losowa wartość wyświetlana w trakcie) — silnik nie wie nic o animacji, dostaje i zwraca finalny wynik natychmiast jak dotychczas; animacja tylko opóźnia moment wyświetlenia tego wyniku na ekranie.
